...
echo 'Download and build binaries'
sleep 0.5
cd $HOME
rm -rf axelar-core
...
# Create Service file
echo 'Create Service file: axelard.service'
sleep 1
...
# Node config
read -r -p "Enter custom port (defaut 173): " CUSTOM_PORT
sleep 0.5
...
curl -L https://snapshots.stakevillage.net/snapshots/axelar-dojo-1/axelar-dojo-1_2024-01-26.tar.lz4 | tar -Ilz4 -xf - -C $HOME/.axelar
sleep 0.5
...
sleep 1 && curl -s https://raw.githubusercontent.com/vnbnode/binaries/main/Logo/logo.sh | bash && sleep 1
echo '====================== SETUP FINISHED ========================================='
echo -e "\e[1;32m Check status: \e[0m\e[1;36m${YELLOW} sudo systemctl status artelad ${NC}\e[0m"
echo -e "\e[1;32m Check logs: \e[0m\e[1;36m${YELLOW} sudo journalctl -u axelard -f --no-hostname -o cat ${NC}\e[0m"
echo -e "\e[1;32m Check synchronization: \e[0m\e[1;36m${YELLOW} axelard status --node http://localhost:17357 2>&1 | jq .SyncInfo ${NC}\e[0m"
